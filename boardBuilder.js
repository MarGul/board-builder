...
jQuery(document).ready(function($) {
	...
	var boardBuilder = (function() {
		...
		var accWrap = $body.find('#accordion');
 		...
 		var objPreview = {};
 		// The coupon text box
 		var coupTxt = $body.find('#couponText');
		...
		var init = function(objPrev) {
			...
			$body.on('change', '.board-font', updateFont);
			// Event for when the discount button is pressed.
			disBtn.on('click', addDiscount);
			// Event for when the coupon text is altered
			coupTxt.on('keyup', resetCoupon);
		}
		...
		var addTextOption = function(objThis) {
			...
			strHtml += 	'<select name="boards[' + boardID + '][font]" class="form-control board-font">';
		...
		var updateFont = function() {
			...
			objPreview.updateFont(index, fontClass);
		}
		...
		var addDiscount = function(){
			...
			$.each(data.coupons, function(index, value) {
				if(value.code == code) {
					...
					//Confirm activation with customer.
					$('#couponDivInner').html('<span class="accepted">Your coupon has been activated!</span>')
				}
			});
			if (found==false){ // If no coupon is found, send an error message
				discount = 0;
				//Inform cutomer of coupon failure.
				$('#couponDivInner').html('<span class="req">This code is not recognized.</span>');
			}
			updateCost();
		} 

		/*
			Resets the coupon text to an empty string
		*/
		var resetCoupon = function(){
			$('#couponDivInner').html('');
			return;
		}
	...
	var boardPreview = (function() {
		...
		return {
			...
			deleteBoard: function(index) {
				$ul.find('.board').eq(index).remove();			
			},
			setTextOptions: function(index) {
				$ul.find('.board').eq(index).removeClass().addClass('board text-functionality block-font').html('');
			},
			setAccessoryOption: function(index, img, qty) {
				$ul.find('.board').eq(index).removeClass().addClass('board accessory-functionality')
									        .html(buildAccessoryHTML(img, qty));
			},
			deleteOptions: function(index) {
				$ul.find('.board').eq(index).removeClass().addClass('board blank-functionality').text('');
			},
			updateText: function(index, text) {
				$ul.find('.board').eq(index).text(text);
			},
			updateFont: function(index, fontClass) {
				$ul.find('.board').eq(index).removeClass('block-font cursive-font').addClass(fontClass);
			},
			changeAccessoryQty: function(index, img, qty) {
				$ul.find('.board').eq(index).html(buildAccessoryHTML(img, qty));
			},
		}
